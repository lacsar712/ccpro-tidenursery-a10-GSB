...
export default function BroodstockCages() {
  ...
  async function move(id: number, direction: 'in' | 'out') {
  ...
  return (
    <div>
      <header className="page-header">
        <h1>亲虾笼位</h1>
        <p className="muted">
          同场笼位号唯一；启用笼位有占用时，该场塘口单次投喂不得超过 2 kg
        </p>
      </header>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id}>
                ...
                <td>
                  <button className="btn ghost" onClick={() => move(r.id, 'in')}>
                    入笼
                  </button>{' '}
                  <button className="btn ghost" onClick={() => move(r.id, 'out')}>
                    出笼
                  </button>{' '}
