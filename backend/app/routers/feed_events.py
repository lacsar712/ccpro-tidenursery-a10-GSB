...
# 亲虾暂养笼位有占用时，该场塘口单次投喂上限（kg）
BROODSTOCK_FEED_LIMIT_KG = 2.0
...
def create_event(
    ...
    if occupied_total > 0 and payload.amount_kg > BROODSTOCK_FEED_LIMIT_KG:
        raise HTTPException(
            status_code=400,
            detail="亲虾暂养笼位仍有占用，本场塘口单次投喂不得超过 2 kg（亲虾占用限喂）",
        )
    item = FeedEvent(
