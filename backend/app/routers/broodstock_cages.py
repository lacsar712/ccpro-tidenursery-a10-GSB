...
@router.get("/{cage_id}", response_model=BroodstockCageOut)
def get_cage(
    cage_id: int,
    db: Session = Depends(get_db),
    _: User = Depends(get_current_user),
...
def stock_in(
    ...
    if item.occupied + payload.count > item.capacity:
        raise HTTPException(status_code=409, detail="超出笼位容纳尾数，无法入笼")
...
def stock_out(
    ...
    if not item:
        raise HTTPException(status_code=404, detail="笼位不存在")
    if item.occupied - payload.count < 0:
